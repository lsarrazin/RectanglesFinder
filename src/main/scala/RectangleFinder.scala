...
object RectangleFinder {
   ...
   */
  def matchRectangles(points: Set[Point]) : Set[(Point, Point, Point, Point)] = {
    if (points.size < 4)
  ...
  def rematchRectangles(newPoint: Point, points: Set[Point], rects: Set[(Point, Point, Point, Point)])
